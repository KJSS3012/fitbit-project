...
watch([() => props.period, () => props.range], () => {
  ...
  data.value = dates.map((date: Date) => ({ date, amount: Math.floor(Math.random() * (max - min + 1)) + min }))
}, { immediate: true })
...
const template = (d: DataRecord) => `${formatDate(d.date)}: ${formatNumber(d.amount)} pacientes`
</script>
